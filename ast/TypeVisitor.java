...
public class TypeVisitor extends Visitor {
        ...
        public Type visit(ArrayAssignment as) {
            ...
            if(t == null) {
                ...
                ErrorMessage e = new ErrorMessage(as.tokenline, as.tokenchar, "variable \"" + as.name + "\" in not an array");
                violations.add(e);
            } else {
                ArrayType tmp = (ArrayType) t;
                if(tmp.type.getClass() != got.getClass()) {
                    ErrorMessage e = new ErrorMessage(as.tokenline, as.tokenchar, "array assignment value of \"" + as.name
                                                      + "\" does not match expected type expected:" + t + " got: " + got);
                    violations.add(e);
                }
            }
            if(index != null && index.getClass() != IntergerType.class) {
                ErrorMessage e = new ErrorMessage(as.tokenline, as.tokenchar, "type of index for \"" + as.name + "\" is not a integer, got " + index);
                violations.add(e);
            }
            return null;
        }
        ...
        public Type visit(IdentifierValue iv) {
            ...
            if(t == null) {
                ErrorMessage e = new ErrorMessage(iv.tokenline, iv.tokenchar, "variable \"" + iv.name + "\" is undefined"); 
                violations.add(e);
            }
            return t;
        }
